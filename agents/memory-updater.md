---
name: memory-updater
description: Mine high-signal transcript deltas, update `CLAUDE.md`, and keep the incremental transcript index in sync.
model: inherit
---
...
3. Inspect only transcript files under `~/.claude/projects/<workspace-slug>/*.jsonl` that are new or have newer mtimes than the index. The `Stop` hook passes the current session's `transcript_path` directly, so start from that file and use the directory only to pick up sessions the index has not seen.
...
- Updated `CLAUDE.md` and `.claude/playbook/continual-learning-index.json` when needed
