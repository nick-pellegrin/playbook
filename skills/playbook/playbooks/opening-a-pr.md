...
**Worktree.** Work from a git worktree off main. Subagents inherit it. Multiple `Agent` calls on the same branch each get their own worktree (spawn them with `isolation: worktree`), or `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
...
**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `playbook` or `watch-pr`, as the scope. Keep the subject short and imperative. Name a real symbol when one carries the change. For example, `fix(playbook): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
**Forge.** All PR operations go through the GitHub CLI (`gh`): create, edit, view, watch, and merge. Do not require Graphite (`gt`). If your team uses a stacked-PR CLI instead, name it here and use it for the same five operations.
...
**Readiness.** Open every PR ready, never as a draft. Omit `--draft` from `gh pr create`. If a PR still opens as a draft, run `gh pr ready <number>`. Run `gh pr view <number>` before you refer to PR status.
