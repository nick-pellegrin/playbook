---
name: recall
description: "Rebuild recent working context from chat history, live state, and the shared record, then hand back a current-state brief. Use for 'catch me up', 'what have I been working on', 'where did I leave off'."
...
Transcripts live at `~/.claude/projects/<slug>/<session-id>.jsonl`, where `<slug>` is this project's absolute path with every separator and colon turned into `-`. Every line is one JSON message.
...
3. Fan out across your chat history. Spawn parallel `explorer` agents, each taking a slice of the corpus. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
